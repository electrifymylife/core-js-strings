...
function removeFirstOccurrences(str, value) {
  ...
  const valueString = valueIndex + value.length;
  const result = str.slice(0, valueIndex) + str.slice(valueString);
...
function isPalindrome(str) {
  const transformToStringLettersOnly = (string) => {
    let lettersOnly = '';
  ...
  const transformToReversed = (string) => {
    let reversed = '';
  ...
  const reversed = transformToReversed(str);
  const reversedLettersOnly = transformToStringLettersOnly(reversed);
  const stringLettersOnly = transformToStringLettersOnly(str);
  return reversedLettersOnly.toLowerCase() === stringLettersOnly.toLowerCase();
}
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((word) => {
    ...
    reversedSentence.push(reversed);
    reversed = '';
  });
...
function encodeToRot13(str) {
  ...
  let alphaLetterIndex = '';
  str.split(' ').forEach((word) => {
    for (let i = 0; i < word.length; i += 1) {
      if (word.charAt(i).toLowerCase() !== word.charAt(i).toUpperCase()) {
        alphaLetterIndex = alphabet.indexOf(word.charAt(i));
        encoded += rot13.slice(alphaLetterIndex, alphaLetterIndex + 1);
      } else {
        encoded += word.charAt(i);
      }
    }
    encodedSentence.push(encoded);
    encoded = '';
  });
